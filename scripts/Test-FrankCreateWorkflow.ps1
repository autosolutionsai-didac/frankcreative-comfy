...
$SmokeScript = Join-Path $Root "scripts\frank_workflow_smoke.py"
...
function Write-Step {
    ...
    Write-Host "[Frank Workflow] $Message" -ForegroundColor $Color
}
...
$ArgsList = @($SmokeScript, "--base-url", $BaseUrl)
if ($KeepSession) {
    $ArgsList += "--keep-session"
}
