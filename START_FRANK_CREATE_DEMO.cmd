...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\Start-FrankCreate.ps1" -ResetDemoData
if errorlevel 1 (
  echo.
  echo Frank Create demo mode did not start. Check user\frank_create\logs for details.
  pause
  exit /b 1
)
echo.
echo Frank Create demo mode is ready. The studio should now be open in your browser.
